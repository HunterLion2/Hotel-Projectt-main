...
class Database
{
    ...
    public function __construct()
    {
        ...
        // DSN (Data Source Name) oluşturur - Socket kullanımı için
        if (strpos($config['DB_HOST'], '/') === 0) {
            // Socket file path ise
            $dsn = "mysql:unix_socket={$config['DB_HOST']};dbname={$config['DB_NAME']};charset={$config['DB_CHARSET']}";
        } else {
            // Normal host ise
